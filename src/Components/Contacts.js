...
import { MDBCard, MDBCardBody, MDBRow, MDBCol, MDBListGroup, MDBListGroupItem, MDBAvatar, MDBBadge, MDBIcon,MDBInput,MDBBtnGroup,MDBPopover,MDBFileInput,MDBCardImage,
  MDBPopoverBody, MDBPopoverHeader,MDBContainer,
...
const Contacts =({switchStatus,Room,Name,Messages,volumeMess,image,status,userId,setMessageCounter,setVolumeMessages,counter,setMessageCounterToZero,token})=> {
...
useEffect(()=>{
      axios.get(`/getMessages/${skip}/${Room}`,{
        headers: {
 ...
 const sendMessage = () => {
   if(message) {
     ...
     socket.emit('sendMessage', {message,userId,msgImage,Room,Name,image}, () => {
       setMessage('')
       setMsgImage("")
  ...
  <MDBCardBody>
    <MDBRow >
      ...
      <MDBRow style={{height:"2.5rem"}} className="d-flex justify-content-center ">
            <MDBBtn onClick={()=>switchStatus()} size={status===false?"sm":null} color={status===false?"white":"red accent-4"} outline rounded  >
              <MDBIcon icon="users" size="lg" />
            </MDBBtn>
            <MDBBtn   size={status===false?null:"sm"} color={status===false?"red accent-4":"white"} outline rounded onClick={()=>switchStatus()}>
              <MDBIcon  fab size="lg" icon="facebook-messenger" />
              {counter!==0?<MDBBadge pill color="amber darken-3">{counter}</MDBBadge>:null}
            </MDBBtn>
            {
              volumeMess===true
              ?
        ...
        <hr/>
        <div className="scrollable-chat" style={{height:"25rem",overflowY:"scroll"}} onScroll={(e)=>onScroll()}  ref={mySecRefer}>
            <MDBListGroup className="list-unstyled pl-3 pr-3" >
              {messages!==undefined? messages.slice(0).reverse().map(message =>
            (  <MDBAnimation type="slideInDown" duration="300ms"><ChatMessage userId={userId} senderId={message.userId} key={message.author + message.when} user={message.user} message={message.text} name={name} room={message.room} Room={Room} msgImage={message.msgImage} Image={message.image} /></MDBAnimation>)
        ...
        <div className="form-group basic-textarea d-flex justify-content-center align-items-center">
          ...
          <MDBPopover
             ...
             id="popper1"
             >
         <MDBBtn floating size="sm" color="amber darken-3" ><MDBIcon   icon="camera" /></MDBBtn>
           <MDBPopoverBody>
           < MDBFileInput
                type="file"
                name="myImage"
       ...
       <MDBPopover
          ...
          id="popper1"
          >
      <MDBBtn floating size="sm" color="amber darken-3"><MDBIcon far size="2x"  icon="grin" /></MDBBtn>
        <MDBPopoverBody>
...
const ChatMessage = ({message,name,user,room,Room,Image,msgImage,senderId,userId}) => {
  ...
  <MDBCard   style={{marginTop:"15px",borderBottomLeftRadius:"28px",borderTopLeftRadius:"28px",borderBottomRightRadius:"28px"}}>
        <MDBCardBody style={{width:"18rem"}}>
        <div style={{fontSize:"0.7em"}}>
          <strong className="primary-font">{user}</strong>
        </div>
        <p className="mb-0" style={{fontSize:"0.75em"}}>{message}</p>
        {msgImage!==""?<MDBCardImage
          top
          src={msgImage}
          alt=""
        />:null}
...
(room===Room ?(
  ...
  <MDBCard  style={{marginTop:"15px",backgroundColor:"#D0322D",borderBottomLeftRadius:"28px",borderTopRightRadius:"28px",borderBottomRightRadius:"28px"}}>
      <MDBCardBody style={{width:"18rem"}}>
        <div className="d-flex algn-self-start" >
          <strong className="primary-font" style={{color:"white",fontSize:"0.7em"}}>{user}</strong>
        </div>
        <p className="mb-0" style={{fontSize:"0.75em",color:"white"}}>{message}</p>
        {msgImage!==""?<MDBCardImage
          top
          src={msgImage}
          alt=""
